...
parameters {
  ...
  real<lower=0> m[nSpecies]; // exponent for swithing
  real<lower=0> n[nSpecies]; // exponent for total ingestion
  
  real<lower=0> sigma;
}
transformed parameters{
  matrix<lower=0>[nData, nSpecies] phi;
  matrix<lower=0, upper=1>[nData, nSpecies] prop;
  matrix<lower=0>[nData, nSpecies] theta;
  ...
  for(i in 1:nData){
    ...
    for(j in 1:nSpecies){
      prop[i, j] = a[j] * N_avail_exp[i, j] / sum(a .* h .* N_avail_exp[i] ); 
      theta[i, j] = sum(a .* h .* N_avail[i] )^n[j] / (1 + sum(a .* h .* N_avail[i] )^n[j] ) ;
      phi[i, j] = prop[i, j] * theta[i, j];
    }
  }
}
model {
